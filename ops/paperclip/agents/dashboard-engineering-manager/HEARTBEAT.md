...
6. Before delegation, implementation handoff, review-ready status, PR, merge, or deploy work, require `pnpm session:preflight` evidence from the task branch. A failing preflight blocks the issue until dirty work or stale refs are preserved and reconciled.
7. For tasks that need GitHub, Context7, or browser/Playwright checks, require `pnpm check:paperclip-runtime` evidence before delegation or final readiness.
8. For production sync/backfill/proof, require the approved GitHub Actions operation surface from `proof-loop.md#production-operation-gate`; block instead of routing normal work to raw SSH.
9. If actionable, create or update the spec and delegate to the smallest correct owner.
10. For non-trivial work, require proof-loop artifacts under `.paperclip/tasks/<issue-id>/` while working and a durable Paperclip/tracked handoff before review.
11. Check whether a reviewer/verifier has fresh context before final readiness.
12. Record self-correction notes when work failed because instructions, tools, ontology, or workflow were insufficient.
13. Do not repeat blocked comments when no new context exists.
14. Do not run weekly/team-development work from this heartbeat file. Weekly work belongs to Paperclip `Routines`, which create separate auditable issues with their own scope, evidence, and report.
